...
- **The contract is the spec.** Requirements are traced in
  `docs/requirements-traceability.md` (codes A1–A4, B1–B14, C1–C13).
  Never invent scope; never silently drop a contracted requirement.
- **`docs/backlog.yaml` is the single source of truth** for scope and
...
- **The API contract is frozen.** `contracts/openapi.snapshot.yaml` is
...
2. For any decision, write an **ADR** in `docs/adr/NNNN-slug.md`:
